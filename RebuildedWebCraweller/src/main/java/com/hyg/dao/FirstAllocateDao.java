...
@Mapper
public interface FirstAllocateDao {
    @Select("select * from first_allocate")
    @Results(
            id = "allocateMap1",
            value = {
    ...
    @Insert("insert first_allocate(fanhao) values(#{fanhao})")
    public abstract void insertAllocate(String fanhao);

    @Delete("delete from first_allocate where fanhao=#{fanhao}")
